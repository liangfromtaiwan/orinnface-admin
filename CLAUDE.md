...
- 指標の版は**分析種別ごとに別物**(吉田さん指摘 2026-09-25)。姿勢の分析に
  ...
  姿勢では undefined。
- 🔴 **登録していない人に名前は無い**(使用者指摘 2026-09-25)。未連携分析の仮データは
  本人が登録していないので誰も名乗っていない。`customerLabel()` / `customerIdLabel()`
  を通し、「未登録の顧客」＋**匿名ID**(§9)で出す。氏名を捏造して出すと、公開 URL で
  実在の人の情報が入っているように見える。`Customer.displayName` は optional。
  ⚠️ 登録済みの表示名の出どころ(本人入力 / Google の表示名 / 取得しない)は
