...
- 企業・店舗を**追加**できるのは **V1 では本部だけ**(吉田さん確定 2026-09-24)。可否は
  ...
  🔴 既にあるものを**直せる範囲は追加とは別**(使用者確定 2026-09-24)。項目ごとに
  `companyEditRights()` / `storeEditRights()` で決める。**契約に関わる項目(企業名・
  契約状態・店舗の開閉)は本部だけ**、契約企業管理者は自社の店舗名と担当者を直せる、
  店舗管理者は担当店舗の担当者(スタッフのみ)だけ。直せない項目は入力欄を出さずに
  今の値と「契約に関わるため、本部だけが変更できます。」を出す。押しても何もできない
  編集ボタンは出さない(1 つも触れないならダイアログごと出さない)。
  ⚠️ この線引きは仕様書に無い。QUESTIONS #25 で確認中
