...
- **下書き・承認済は削除できる**（使用者確定 2026-09-18）: 編集ダイアログの中に「この版を削除する」。一度押しただけでは消えず、理由必須（§13）。監査は `deletion`。**有効・退役は消せない**（退役は過去の推奨の根拠。消すと「なぜその推奨が出たか」を答えられなくなる）。画面に出さないだけでなく `applyBaselineDelete()` でも弾く。
