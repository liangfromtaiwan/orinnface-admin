...
| ☐ | 本部が提供者・内容・権利・承認状態・公開期間・対象 scope を確認し approve 後に assignment を予約 | `CareRequestReviewDialog`（行クリックで開く。今この範囲に出ている動画と並べて再生・承認/却下） | ⚠️ | **「内容」を見られるようにした**（2026-09-15）。一覧に承認ボタンは置かず、確認ダイアログからのみ承認できる。権利未確認は承認不可・却下は理由必須。ただし **approve が toast のみで永続化しない**のは変わらず（backend 担当） |
| ☐ | 適用範囲の表示 | `careScopeLabel()` | ✅ | 「会社全体」ではなく「◯◯社 の全店舗」「◯◯店 のみ」。本部は全社横断で見るので会社名を落とさない |
| ☐ | 有効日時で care_asset_id だけ切替。video_code / pose_code は不変 | 型で固定 | ✅ | |
| ☐ | 元 asset・差し替え asset・申請者・承認者・理由・開始終了・取消・catalog version を履歴保持 | `types.ts` に項目あり | ⚠️ | 型と seed はあるが**履歴画面が無い**（rollback 用の過去一覧） |
