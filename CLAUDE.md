...
- 企業・店舗を**追加**できるのは **V1 では本部だけ**(吉田さん確定 2026-09-24)。可否は
  ...
  🔴 既にあるものを直せるのも**本部だけ**(使用者確定 2026-09-24)。**企業名・店舗名・
  契約状態・店舗の開閉は契約に関わる**(店舗名は契約書上の名称)ので、契約企業側からは
  変えられない。契約企業管理者・店舗管理者にできるのは**担当者の追加・解除だけ**で、
  それは一覧の「担当者」列から入る。判定は項目ごとに `companyEditRights()` /
  `storeEditRights()`。直せない項目は入力欄を出さずに今の値と「契約に関わるため、
  本部だけが変更できます。」を出し、1 つも直せないなら編集ボタンごと出さない
  (読み取り専用のダイアログを開かせない)。
  ⚠️ 「一時休業」のような状態が現場に要るかは未決 (QUESTIONS #25)
