...
- 畫面顯示統一用「**企業**」，不用「会社」(使用者確定 2026-09-22)。仕様書的「契約企業」
  用語一致。畫面名稱是「企業・店舗」。內部 code 的 `company` / `companyId` 不改。
...
- 🔴 **同意は連携より先に取る**。同意なしに `store_data_link` が作られることはない。
  → `StoreDataLink.consentedAt` は**必須**。optional にすると
  「連携はあるが同意がない」という起こらない状態を表現でき、各画面で
  その分岐を持つことになる(以前は「再同意待ち」バッジを持っていたが削除した)。
- 連携解除後は閲覧不可。再び閲覧するには**再連携**が必要で、そのとき以前の同意は
  引き継がず取り直す(吉田さん確定 2026-09-07)。解除済みの連携も当時の同意日時は残す。
- 閲覧可否の判定は `status` で決まる。`canViewCustomer()` で同意を再判定しない。

### 顧客の連絡先

- 🔴 メールアドレスは **identity 側**(`CustomerIdentity`)に置く。`Customer` 型に持たせない
  (§5「analytics へ PII を混入しない」)。画面では `dataSubjectId` で join して出す。
- 🔴 **出どころを一緒に出す**(`CustomerContact`)。未登録顧客のメールは撮影前に
  店舗スタッフが**簡易登録**で入力したもので、本人のアカウントはまだ無い(未連携)。
  本人が登録したものと同じ顔で出すと、連絡してよい先だと読まれる。
  `CustomerIdentity.source` が `account` / `store_intake` を持ち、`accountId` は
  簡易登録の段階では付かない。
- Guest(未ログイン)は連絡先を持たない → 「メールアドレスなし（Guest・未ログイン）」。
...
- 審査の可否は `decideCareRequestAction()`、反映は `applyCareRequestAction()`。
  🔴 **審査できるのは本部だけ**(§4.2 契約企業・店舗は申請のみ)。本部以外には承認・却下の
  ボタンを**出さない**。押せないボタンを並べても、誰が審査するのかが伝わらない。
  🔴 承認は即公開とは限らない。開始日時が未来なら「公開予約」。即時公開なら同じ枠・
  同じ範囲で公開中だったものを `ended` にする(§13 重複有効の禁止)。
  🔴 申請理由 `reason` と審査理由 `decisionReason` は別の欄。1 つに上書きすると
  「誰の言い分か」が消える(§7.1 の履歴要件は申請者・承認者・理由をすべて残すこと)
- 差し替え申請・履歴の一覧は `visibleCareRequests()` を通す。
  🔴 本部以外には**自分に効くものだけ**。他社の申請が見えると、どこがどの動画を
  使っているかが漏れる。本部デフォルトは自店にも出ているので誰にでも見える
...
- 指標の名前・構成は**ユーザー向け結果画面に合わせる**(2026-09-21 確認)。
  動作名は 口角挙上 / 口すぼめ / 開口 / 開眼 / 眉間収縮。左右差は動作ごとに測っている
  部位が違うので名前も別(いー笑顔 / 口中心 / 顎 / 目 / 眉の高さ)。代償があるのは
  **開眼と眉間収縮の 2 動作だけ**。無表情は画面と同じ **3 指標**(左右差：目の高さ /
  左右差：口角 / 口角の下がり)。⚠️ §5 は「6 指標」と書いているが画面が 3 つなので
  画面を正とした(QUESTIONS #23 で確認中)
- 指標そのもの(metric_code)は**管理画面から増減しない**。AI が返すものがカタログ。
  増やしても欠測の行が増えるだけで、§5 の同一指標原則も崩れる。本部が確認できるよう
  推奨設定の「指標一覧」タブに**表示のみ**で出してある
- 分析結果の表示は**結果画面と同じ区切り**にする(無表情 / い ー / う ー / あ ー / 目 /
  眉間 の 6 セクション)。🔴 指標の種類(可動域・左右差・代償)ごとに分けない。1 つの動作の
  話をするのに複数のカードを見比べることになり、「私の口すぼめが要ケアなのはなぜ」に
  答えられない。判定は `thresholds.ts` から引く(閾値は暫定)。代償は数値を出さず なし / あり
- 筋肉タグ(`muscles.ts`)は「**その動作に関係する筋肉を示す表示用タグ**」
  (吉田さん確定 2026-09-24 の文言。言い換えない)。🔴 **表示用のみ**で、AI 分析・判定・
  推奨の順位には影響させない。本部だけが追加・削除・改名できる。
  🔴 押した瞬間に反映しない。編集をためて**保存の前に差分を見せる**(変更確認)。
  保存すると**履歴**に残る。ユーザーの結果画面に出るものなので、意図しない変更に
  気付けないと困る。
  🔴 同じ筋肉が複数の動作に付くので、**改名は全動作まとめて**行う。差分でも改名は
  1 件として出す(削除 2 + 追加 2 に見せない)。
  🔴 1 動作あたりの**上限は決め打ちしない**(吉田さん確定 2026-09-25「未確定なので
  上限を決め打ちせず確認待ちに」)。`MAX_TAGS_PER_POSE` は undefined のままにし、
  件数では弾かない。画面には「上限は未確定」と出す
- 顔の指標の単位は **pt**(使用者確定 2026-09-21)。ユーザー向け結果画面が pt で出して
  いるので、管理画面も同じ単位・同じ値にする (§5 同一指標原則)。姿勢は mm / deg のまま。
  🔴 **判定閾値(正常/要注意/要ケア)と推奨基準値は別物**。前者は結果画面の色を決め、
  正本は AI分析 v1.6(管理画面からは変えられない・`thresholds.ts` は表示用の暫定値)。
  後者はどの 2 動作を勧めるかを決める(§8・管理画面で版管理する)。混ぜない
- 権利確認は `decideRightsClear()` を通す。🔴 確認できるのは**本部だけ**(§7.1)。
  未確認の動画は公開できない(差し替えの承認が拒否される)。**中身を再生してから**
  確認させる(§7.1 は「提供者・内容・権利」を確認すると定めている)。誰がいつ何を
  もって確認したかを asset と監査の両方に残す。⚠️ 確認済を**取り消す**操作は無い
  (契約が切れた動画の扱いが未決 → QUESTIONS #24)
...
- 推奨の版操作は `decideDraftCreate()` / `decideSetAction()` を通す。画面側で role を
  ...
  一度押しただけでは消えないようにして理由を必須にする(§13)。監査は deletion。
  **有効・退役は消さない**。退役した版は過去の推奨の根拠で、消すと「なぜその推奨が
  出たか」を答えられなくなる。関数側でも弾く。
  🔴 下書き・承認済は**中身を直せる**(`decideSetEdit()`・使用者確定 2026-09-18)。
  §8 が禁じているのは **active 値の直接更新**であって、まだ有効化していない版では
  ない。有効・退役は画面にも関数にも編集の口を出さない。承認済を直したら下書きへ
  戻し、有効化の予約も外す(そのままだと編集後の内容が予約時刻に有効化される)。
  🔴 **承認は操作として置かない**(使用者確定 2026-09-18)。本部が 1 人なら同じ人が
  もう一度押すだけの手順にしかならない。下書きからそのまま有効化・予約できる。
  誰が決めたかは approvedBy に記録する。承認済という状態は残り、**予約したときに
  入る**(下書きのまま予約を持たせると、予約時刻にまだ決めていない版が有効化される)。
  🔴 画面の表示名は「**有効化待ち**」。値は approved のままだが、承認という操作が
  無いのに「承認済」と出すと「誰がどこで承認したのか」を探させる(実際に 2 度聞かれた)。
  承認者の表示も「決定 ◯◯」にしてある。
  → §8 の操作表には approve があるので、意図的な逸脱として吉田さんに共有済み。
  🔴 作成者と承認者は**分けない**(使用者確定 2026-09-18)。本部の管理者が自分で決めて
  よい範囲なので、自分が作った下書きをそのまま承認できる。§8 の「分離を推奨」は必須で
  はないため警告も出さない。
...
- 理由には最低文字数がある(`reason-rules.ts` の 4 文字)。🔴 **弾く条件は画面に書く**。
  「あと N 文字」を入力欄の下に出し、送信ボタンにも title を付ける。条件を書かずに
  ボタンだけ無効にすると、何が足りないのか分からないまま止まる(実際に起きた)。
- 本部メンバーは本部だけが増やせる(吉田さん確定 2026-09-24)。operator の membership も
  `MembershipTarget` の company として持つ(本部にも内部的に company がある。role だけが違う)。
  🔴 アカウントは**1 人 1 つ**。共有すると監査の「誰がやったか」が消える。
  🔴 2FA は operator 必須だが**設定するのは本人**。招待直後は未設定なので、一覧に
  「2段階認証 未設定」と出す。自分の担当は自分で外せない
- 企業・店舗を**追加**できるのは **V1 では本部だけ**(吉田さん確定 2026-09-24)。可否は
  `canEditOrganizations()`。スタッフの招待は別で、企業管理者・店舗管理者が権限範囲内で行う。
  🔴 既にあるものを直せる範囲は項目ごと(使用者確定 2026-09-24)。判定は
  `companyEditRights()` / `storeEditRights()`。**名前(企業名・店舗名)と企業の契約状態は
  本部だけ**——どちらも契約書上のものだから。**店舗の営業状態(営業中 / 閉店)は
  契約企業管理者も自社の店舗なら変えられる**(休業・閉店は現場で起きる。閉店は理由必須)。
  店舗管理者は店舗そのものは直せず、担当者(スタッフ)だけ。担当者の可否は
  `decideMembershipEdit()` が正本。直せない項目は入力欄を出さずに今の値と
  「契約に関わるため、本部だけが変更できます。」を出し、1 つも直せないなら編集ボタン
  ごと出さない(読み取り専用のダイアログを開かせない)。
  ⚠️ 「一時休業」のような状態を足すかは未決 (QUESTIONS #25)
  🔴 **解約・一時停止は消さずに状態を変える**。顧客・分析履歴・同意・保存期限を作り直さない
  (§2)。削除の導線は作らない。企業を止めたら配下の店舗もまとめて止める(店舗だけ動いたまま
  だと契約が切れているのに撮影できる)。ただし契約中へ戻すときに店舗を勝手に開けない。
  ⚠️ 監査カテゴリ `organization_change` は §11 の一覧に無く、こちらで足したもの
- 監査に残る操作の理由入力は `ReasonField` を通す。🔴 必須であることを入力欄の側で
  先に伝える(「必須」バッジ＋未入力は枠を警告色)。押せないボタンだけで伝えると、
  押してから気づくことになる。理由は §11 の変更監査にそのまま残る
- membership の付与・剥奪は `decideMembershipEdit()` を通す。画面側で role を
  ...
  招待は `decideInvite()`。招待できる範囲は付与できる範囲と同じ。
  🔴 招待したアカウントは `status: "invited"` で、本人がパスワードを設定するまで
  使えない(ログイン切替にも出さない)。パスワードは招待した側が触らない。
  同じメールなら**アカウントを作り直さず担当だけ足す**(兼任があるため)
  `applyMembershipChange()` は membership 行の足し引きだけを行い、
  account・顧客・分析履歴・同意・保存期限は作り直さない (§2)

- 表は**空のときに必ず理由を出す**(`TableEmpty`)。ヘッダーだけの表は「壊れている /
  読み込み中」に見える(実際に「保持の画面が空白」と指摘を受けた 2026-09-25)。
  絞り込みの結果なら、その場で解除できるようにする。
  🔴 画像・保持の検索は**表示名では探せない**(§5 analytics に PII を混ぜない)。
  0 件のときは探し方まで返す。
- 指標の版は**分析種別ごとに別物**(吉田さん指摘 2026-09-25)。姿勢の分析に
  `face-v1.6.0` を出さない。同年代平均(average_version)は無表情だけに付くので
  姿勢では undefined。
- 名前の出どころは 2 つ(店舗スタッフ画面の設計 2026-09-25)。**未登録顧客**は
  店舗スタッフが撮影前の**簡易登録**(お名前・メール・年齢)を入れるので名前がある。
  **Member / Premium** は本人が登録している。
  🔴 **名前を持たないのは Guest(未ログイン)だけ**。無い人に氏名を作らない
  (公開 URL で実在の人の情報に見える)。`customerLabel()` が「Guest（未ログイン）」を
  返し、識別子は `customerIdLabel()` が匿名ID を出す。`Customer.displayName` は optional。
  ⚠️ 本人登録時の表示名の出どころ(本人入力 / Google の表示名 / 取得しない)は
  アカウント認証 v1.0 待ち → QUESTIONS #26
- 🔴 **Guest はログインしていない利用者**(使用者確定 2026-09-25)。1 日 1 回の分析のみ。
  名前も連絡先も持たない。**店舗連携の前に必ずログインが要り、ログインすると Member**
  になるので、Guest のまま連携済みにはならない(seed も `storeDataLinks` を作らない・
  `npm run smoke` で検証)。名前を持たない状態が 2 種類あるので語を変える:
  「Guest（未ログイン）」= B2C で自分で撮った人 /「未登録の顧客」= 店舗で撮ったが
  登録していない人(未連携分析)。どちらも匿名ID で並べる。
  ⚠️ 吉田さんの「Guest 会員が店舗連携済みはあり得る」とは食い違うので確認中
  (QUESTIONS #26)
- ブランドロゴは **PNG / WebP・2MB 以下**(吉田さん確定 2026-09-25)。
  `checkLogoFile()` で弾き、`validateBranding()` は反映の直前にもう一度見る。
  🔴 SVG は受け付けない(スクリプトを埋め込めるため)。backend でも同じ検査が要る。
- care 動画の追加ボタンは「**動画素材を登録**」(吉田さん指摘 2026-09-25)。
  「動画を追加」と書くと枠が増えると読める。増えるのは asset で、13 枠は固定。
- 公開 URL で確認してもらうので、**画面に出るデータは架空のサンプルのまま**にする
  (吉田さん指摘 2026-09-25)。ヘッダーに「デモ環境・架空のサンプルデータ」を常設し、
  ログイン画面にも同じ注記を置く。
  🔴 `npm run smoke` が seed 全体を走査して守る: メールは example.jp / example.com だけ /
  電話番号・郵便番号らしき文字列が無い / **顧客の表示名は `SAMPLE_FAMILY_NAMES` ×
  `SAMPLE_GIVEN_NAMES` の組み合わせだけ**(実データを貼り込んだら落ちる)。
  🔴 検索エンジンに載せない。`public/robots.txt`・`index.html` の meta robots・
  `vercel.json` の `X-Robots-Tag` の 3 箇所。どれか 1 つでは漏れる
  (robots.txt はクロールを止めるだけ、meta はクロールされて初めて読まれる)。

### レイアウト
- 🔴 幅の広い表は**表だけが横スクロール**する。ページ自体を広げない。
  flex アイテムは `min-w-0` が無いと中身より狭くならないので、`SidebarInset` と
  Layout の本文列に `min-w-0` を入れてある。これを外すと main が画面より広がり、
  ヘッダー右上のボタンが画面外に出る。
- 🔴 `PageHeader` の操作ボタンは**画面幅によらず右上**。見出しと同じ行に固定し、
  説明文はその下で全幅を使う。説明文と同じ行に並べると、幅が狭いときにボタンが
  下へ回り込んで探さないと見つからない位置へ動く。
- 🔴 **タブがある画面は、操作ボタンをタブと同じ行の右端に置く**(care動画・推奨設定)。
  見出し行と分けることで、ボタンが「どの一覧に対する操作か」が見て分かる。
  タブが無い画面は上の通り見出しと同じ行。2 つの流儀を画面ごとに混ぜない。
...
招待メールの送信、CSV export(§11 に監査カテゴリ export はあるが画面に導線が無い)、
管理画面のビジュアル(§16 P2)。
🔴 画面から変えたものは**すべて React state 止まりでリロードすると消える**
(membership・企業・店舗・差し替え申請の審査・推奨の版操作・筋肉タグ・ブランド設定)。
永続化は backend 担当。この repo はフロントエンドのみ。
...
1. 🔴 分析の「品質」の定義(等級・閾値・適格分析から除外するか) ← **KPI の母数に影響**／AI 側の検討待ち
...
8. ✅ 既存 Premium 会員が店舗連携しても**本人の契約と課金は継続**し Premium 会員数に含める。
   店舗連携・B2B 契約は**別契約**(吉田 2026-09-07)。care 権限は 2 軸（店舗提供＝プラン不問／標準＝プラン判定）
8-2. ✅ 未連携分析も B2B 課金に含める(吉田 2026-09-07)。ただし登録・連携後に二重計上しない
...
14. ✅ **店舗管理者はスタッフを追加できる**(吉田 2026-09-14)。§4.3 の「確認する」は実態と不一致
    ← 増やせるのはスタッフのみ。同格の店舗管理者は増やせない
...
18. ✅ 「作成者と承認者の分離」は**しない**(使用者確定 2026-09-18)
    ← 本部の管理者が自分で決めてよい範囲。§8 の「推奨」に対する逸脱として吉田さんに共有する
19. 🟡 有効化の「予約」を実行するのは誰か・job 失敗をどこで気付くか(2026-09-15 起票)
    ← 同じ版の予約は有効化時に消す実装。別の版の予約は消していない(backend と要すり合わせ)
20. 🟡 care 動画 asset の「持ち主」をどう持つか(2026-09-18 起票)
    ← assignment には会社・店舗が付くが asset には無い。登録済み動画の一覧は本部のみに
    したが、差し替えダイアログの候補は持ち主で絞れていない
24. 🟡 **権利確認は何をもって確認済とするか**(2026-09-22 起票)
    ← 本部が中身を見て確認できる導線は実装済。契約書の保管先・確認済の取り消しが未決
23. 🟡 **結果画面と仕様書で指標の数が合わない**(2026-09-22 起票)
    ← 無表情 6 vs 画面 3 / 代償は 2 動作だけ / 筋肉タグの正本と監査カテゴリ
22. 🔴 **判定閾値の正本と単位**(2026-09-21 起票)
    ← 顔の指標は pt に統一済(結果画面の表記に合わせた)。閾値は推奨設定の「判定閾値」
    タブに**表示のみ**で出しているが、数値は結果画面からの逆算で暫定。正本は AI分析 v1.6
21. ✅ **本部は開始時 1 名。後から追加できるように**(吉田さん確定 2026-09-24)
    ← 各自の個別アカウント・2FA 必須。企業・店舗画面に「本部メンバー」を実装済み
