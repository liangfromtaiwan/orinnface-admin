...
- [x] `ReasonField` ✅ 2026-09-18（2026-09-21: 最低 4 文字。足りないと「あと N 文字」を出す。判定は `reason-rules.ts`） - 監査に残る操作の理由入力(§13)。「必須」バッジ＋未入力は枠を警告色に。理由を使う画面はすべてこれを通す(押せないボタンだけで必須を伝えない)
