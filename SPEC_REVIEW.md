...
| **G→M(無料登録)追跡** | ⚠️ | 現在追跡無し、規格質問 #5(DEMO.md)で雇主確認中 |
...
#### ❌ 残存 mismatch(コード内のテキスト)

| 該当箇所 | 内容 | 修正方針 |
|---------|------|---------|
| `src/pages/ContentPage.tsx:92` | StatCard description が `"5 カテゴリ × 3 尺(30/60/120秒)"` のまま | データは 12 本/2 尺に修正済みだが、description テキストが古い。`"5 カテゴリ × 2 尺(30/60秒)"` 等に修正必要 |

### 影響中 — ⚠️ 雇主確認待ち

| 規格 v2 項目 | 影響範囲 | 現状 | 確認事項 |
|--------------|---------|------|---------|
| 初回ユーザー情報(性別・年代・疲れやすさ・表情こわばり) | 管理画面に年代別 / 性別別分布 KPI を追加するか | 0 実装、User 型にも該当 field 無し | 管理画面で属性別分析が必要か?v1.0 か v2.0 か? |
| Premium CTA タイミング(Day 7 / ケア後 / Day 30) | 転換率 KPI、CTA 効果分析 | 0 実装 | 管理画面で CTA 効果分析が必要か?それとも別系統(マーケ用)? |
| 価格変更(¥980 → ¥780) | ステータス画面など | **管理画面に価格表示は一切無し**(grep 結果 0 件) | 表示しないことで合意か?将来「課金額シミュレーション」のような機能を追加する場合は反映必要 |
| G→M 純増(無料登録ファネル) | プラン推移 KPI | 営収シグナル(Premium 加入/離脱)のみ追跡 | 規格質問 #5(DEMO.md)で確認中 |
| 全体コンディションスコアの定義 | B2BDashboard | AI 疲労 5 段階の Premium 限定算術平均 | 規格質問 #2(DEMO.md)で確認中。複合スコアにするか? |
...
|------|------|
| ✅ 実装済 | 52 |
| ⚠️ 確認待ち | 6(うち 5 件は v2 影響中、1 件は v1 由来) |
| ❌ Mismatch | 1(`ContentPage.tsx:92` description のみ) |
