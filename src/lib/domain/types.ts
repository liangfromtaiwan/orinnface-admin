...
export type StoreDataLink = {
   ...
   * この連携に対する本人の同意日時。
   *
   * 🔴 同意は**連携より先**に取る。同意なしに連携が作られることはないため必須。
   *    optional にすると「連携はあるが同意がない」という起こらない状態を
   *    表現できてしまい、その分岐を各画面で持つことになる。
   * 🔴 連携解除後は閲覧不可。再び閲覧するには**再連携**が必要で、そのとき
   *    以前の同意は引き継がず改めて取り直す (吉田さん確定 2026-09-07)。
   *    解除済みの連携も、当時の同意日時はそのまま残す。
   */
  consentedAt: string
}
