...
#### 5-2 警告色位置重新評估(2026-05-13)

**問題**:規格書 3-1 寫「主観 vs AI 落差 → 行を赤くする + 警告アイコン」,
但沒明確指定哪個畫面實作。

**初步詮釋(已捨棄)**:
原計畫做 Dashboard 上的「要注目ユーザー」獨立區塊,
但 deeper review 規格後判定這是「加值設計」,非規格要求。

**重新評估**:
重讀規格後,「行を赤くする」的「行」最合理解讀是
ユーザー詳細頁的「全行動履歴」列表中的某行。理由:
- 規格書 3 節明確列出「ユーザー詳細」這個畫面
- 「全行動履歴」是個列表(有「行」可變紅)
- 「何か起きている」是事件層級語感,與「該次分析記錄」吻合
- 規格沒有任何 Dashboard 加上人物層級警告的描述

**最終決策**:
- 警告色實作集中在 ユーザー 詳細頁的行動歷史列表(5-2B)
- Dashboard 不加任何人物層級的警告區塊
- Day 5 範圍擴大:新增 ユーザー一覽頁(5-2A)+ 詳細頁雛形(5-2B)
- 下週一開會時主動跟雇主確認此解讀

**影響範圍**:
- 取消原 5-2 計畫(Dashboard AttentionUsersTable 區塊)
- 新增 5-2A(ユーザー一覽頁)、5-2B(詳細頁警告色)、5-2C(配套)
- 保留 types.ts 中的 fatigue 落差判定 helpers(`hasFatigueGap` / `fatigueGap`,5-2B 會用)
...
- 規格模糊處要主動質疑,不要憑直覺實作
- 「重新詮釋規格」比「完成功能」更重要
- 寫過的 code 該砍就砍,別捨不得沉沒成本
- 分階段 commit 是設計工具,不只是存檔
- 親自測試邊界情況(b2b leak)抓出隱藏問題

完成度:Day 5 主體功能 100% 落地
進度:83% (5/6)

Day 6 計畫:
- 部署到 Vercel
- 撰寫 DEMO.md(給雇主)
- 用 production URL 全功能 QA
- 下週一 demo 順序準備
...
#### 規格 4-4「将来対応」三項處理(2026-05-15)

依規格書 4-4 章節,有三項標註「将来対応」(未來對應):

- **OEM company_id 結構**:✅ 已實作(Day 2)
  - multi-tenant 已建立、subType 欄位已設(operator / shop / influencer / company)

- **多言語對應(messages.lang)**:🅿️ 不實作
  - 規格指針對 messages table 的多語系
  - 雇主於 Day 2 取消文言管理,此項連動失效
  - 下週一可確認是否仍需要(預期不需要)
